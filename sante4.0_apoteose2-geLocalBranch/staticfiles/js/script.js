...
function calculateAge() {
    inputed_date = document.getElementById('id_date_naissance').valueAsDate.getFullYear();
    today = new Date().getFullYear();
    age = today - inputed_date;
    document.getElementById('calculated-age').innerHTML = age;
...
function checkFRAX() {
    ...
    if (age <= 24) {
        ...
        for (let id of ['z-score-col-femur', 'z-score-hanche-totale', 'z-score-rachisi-lombaire', 'z-score-extremite-distale']) {
            let element = document.getElementById(id).innerHTML;

            element = element.replace(element[0], 'T');
        }
    }
}
